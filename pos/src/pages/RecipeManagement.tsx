...
const ItemSearch: React.FC<ItemSearchProps> = ({ value, placeholder = 'Search item…', onSelect, onClear, className = '' }) => {
  ...
  const search = useCallback(async (q: string) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/resource/Item?fields=["item_code","item_name","stock_uom","valuation_rate"]&filters=[["item_name","like","%25${encodeURIComponent(q)}%25"]]&limit_page_length=20`);
...
interface RecipeForm { name:string; item_name:string; item:string; item_uom:string; description:string; quantity:number; uom:string; sell_price:number; linked_menu_item:string; linked_menu_item_name:string; status:string; items:IngredientRow[]; }
const emptyForm = (): RecipeForm => ({ name:'', item_name:'', item:'', item_uom:'Nos', description:'', quantity:1, uom:'Nos', sell_price:0, linked_menu_item:'', linked_menu_item_name:'', status:'Available', items:[{ ...emptyIngredient(), _id:Date.now() }] });

// ── Modal ─────────────────────────────────────────────────────────────────────
interface ModalProps { title:string; onClose:()=>void; onSubmit:()=>Promise<void>; submitLabel:string; form:RecipeForm; setForm:React.Dispatch<React.SetStateAction<RecipeForm>>; saving:boolean; }

const RecipeModal: React.FC<ModalProps> = ({ title, onClose, onSubmit, submitLabel, form, setForm, saving }) => {
...
const RecipeManagement: React.FC = () => {
  ...
  const fetchRecipes = async () => {
    try {
      ...
      while (hasMore) {
        const res = await fetch(`/api/resource/BOM?fields=["*"]&limit_start=${start}&limit_page_length=100`);
        const data = await res.json();
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      const payload = { item:itemCode, item_name:addForm.item_name, docstatus:1, description:addForm.description||addForm.item_name, quantity:addForm.quantity||1, uom:addForm.uom||'Nos', is_active:1, is_default:1, items:addForm.items.filter(i => i.item_code.trim()!=='').map(i => ({ item_code:i.item_code, item_name:i.item_name, qty:i.qty, uom:i.uom||i.stock_uom||'Nos', rate:i.rate||0 })) };
  ...
  const handleUpdate = async () => {
    ...
    setSaving(true);
    try {
      const payload = { bom_name:editTarget, description:editForm.description, docstatus:1, quantity:editForm.quantity, items:editForm.items.filter(i => i.item_code.trim()!=='').map(i => ({ item_code:i.item_code, item_name:i.item_name, qty:i.qty, uom:i.uom||i.stock_uom||'Nos', rate:i.rate||0 })) };
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-64px)] overflow-hidden bg-gray-50/80">

      {/* Header */}
      <div className="shrink-0 bg-white border-b border-gray-100 shadow-sm px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/')}
            ...
            <Monitor className="h-4 w-4" /> EPOS
          </button>
          <div>
            <h1 className="text-xl font-extrabold text-[#2D2A26] tracking-tight">Recipe Management</h1>
            <p className="text-xs text-gray-400 font-medium mt-0.5">{recipes.length} recipe{recipes.length !== 1 ? 's' : ''} · Manage BOMs and link to menu items</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
      ...
      {showAdd && (
        <RecipeModal title="Add New Recipe" onClose={() => { setShowAdd(false); setAddForm(emptyForm()); }}
          onSubmit={handleAdd} submitLabel="Create Recipe" form={addForm} setForm={setAddForm} saving={saving} />
      )}
      {editTarget && (
        <RecipeModal title="Edit Recipe" onClose={() => { setEditTarget(null); setEditForm(emptyForm()); }}
          onSubmit={handleUpdate} submitLabel="Update Recipe" form={editForm} setForm={setEditForm} saving={saving} />
      )}
    </div>
  );
};
