...
import { X, ChevronLeft, ChevronRight, MapPin, Tags, Search, Plus, Edit, Trash2, ChevronRight as ChevronRightIcon } from 'lucide-react';
...
interface InventoryManagementCategoriesTourProps {
  isOpen: boolean;
  onClose: () => void;
  onComplete?: () => void;
}

const InventoryManagementCategoriesTour: React.FC<InventoryManagementCategoriesTourProps> = ({ isOpen, onClose, onComplete }) => {
  ...
  const tourSteps: TourStep[] = [
    {
      ...
      description: 'Click to return to the Stock Items view from the Categories page.',
    ...
    },
    {
      id: 'add-category',
      title: 'Add Category',
      description: 'Click to open the Add Category modal to create a new category.',
      target: '[data-tour="add-category"]',
      icon: <Plus className="w-5 h-5" />,
      position: 'bottom',
      action: () => {
        const addButton = document.querySelector('[data-tour="add-category"]') as HTMLButtonElement;
    ...
    },
    {
      ...
      description: 'Click to refresh the categories list and fetch latest data from the server.',
    ...
    },
    {
      id: 'search-bar',
      title: 'Search Bar',
      description: 'Search categories by name, display name, or description. Results update in real-time.',
    ...
    },
    {
      ...
      target: '[data-tour="category-name"]',
      icon: <Tags className="w-5 h-5" />,
      position: 'bottom',
      action: () => {
        const modal = document.querySelector('[data-tour="category-name"]');
        if (!modal) {
          const addButton = document.querySelector('[data-tour="add-category"]') as HTMLButtonElement;
          if (addButton) addButton.click();
        }
    ...
    },
    {
      ...
      target: '[data-tour="category-description"]',
      icon: <Tags className="w-5 h-5" />,
      position: 'bottom'
    },
    {
      ...
      target: '[data-tour="parent-category"]',
      icon: <Tags className="w-5 h-5" />,
      position: 'bottom'
    },
    {
    ...
    },
    {
      id: 'save-category',
      title: 'Save Category',
      description: 'Click to save the category. The button is disabled until required fields are filled.',
      target: '[data-tour="save-category"]',
      icon: <Plus className="w-5 h-5" />,
      position: 'bottom'
    }
  ];
  ...
  useEffect(() => {
    if (isOpen && currentTourStep) {
      ...
      const modalSteps = ['category-name', 'display-name', 'category-description', 'parent-category', 'is-group', 'save-category'];
...
export default InventoryManagementCategoriesTour;
