...
export async function frappeFetch(input: RequestInfo | URL, init: RequestInit = {}): Promise<Response> {
	const csrfFromCookie = getCookie('sid') || '';
	const csrfFromWindow = (typeof window !== 'undefined' && (window as any).csrf_token) ? (window as any).csrf_token : '';
	const csrfToken = csrfFromCookie || csrfFromWindow;
